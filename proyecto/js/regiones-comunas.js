...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // La variable 'regionesChile' viene del archivo regiones-comunas.js
    regionesChile.forEach(region => {
        const option = document.createElement('option');
        option.value = region.id; // Usamos el ID de la región como valor
        option.textContent = region.nombre;
        regionSelect.appendChild(option);
    });

    // Evento que se dispara cuando el usuario cambia la región seleccionada
    regionSelect.addEventListener('change', function() {
        // Limpiar el select de comunas
        comunaSelect.innerHTML = '<option value="" selected disabled>Primero seleccione una región</option>';

        // Obtener el ID de la región seleccionada
        const regionIdSeleccionada = parseInt(this.value);

        if (regionIdSeleccionada) {
            // Encontrar la región correspondiente en el arreglo de datos
            const regionEncontrada = regionesChile.find(r => r.id === regionIdSeleccionada);
            ...
            if (regionEncontrada) {
                ...
                // Cargar las comunas de la región encontrada
            ...
            // Si no hay región seleccionada, deshabilitar el select de comunas
